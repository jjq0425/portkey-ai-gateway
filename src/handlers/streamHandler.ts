...
export async function handleNonStreamingMode(
  ...
  // [ir-intercept] 可选 Hono Context，用于跳过自身代发请求的回环拦截
  c?: any
): Promise<{
  response: Response;
  ...
  if (responseBodyJson && typeof responseBodyJson === 'object') {
    ...
    try {
      await irInterceptNonStreamingJson(responseBodyJson, gatewayRequest, c);
    } catch (e) {
      console.error('[ir-intercept] non-streaming intercept failed:', e);
    }
  }
  return {
    ...
    (async () => {
      try {
        ...
        for await (const chunk of readAWSStream(
          ...
          await writer.write(encoder.encode(chunk)); // [tool-hijack DISABLED]
        }
      } catch (error) {
  ...
  if (
    ...
    (async () => {
      ...
      while (true) {
        ...
        await writer.write(encoder.encode(chunk.value)); // [tool-hijack DISABLED]
      }
      writer.close();
    })();
  } else {
